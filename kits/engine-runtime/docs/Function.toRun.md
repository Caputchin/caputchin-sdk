...
> **toRun**\<`S`, `A`, `C`, `V`\>(`engine`, `opts`): (`seed`, `config`, `trace`) => [`Verdict`](Interface.Verdict.md)
...
| `opts` | [`ToRunOptions`](Interface.ToRunOptions.md)\<`A`\> |

## Returns

(`seed`, `config`, `trace`) => [`Verdict`](Interface.Verdict.md)
