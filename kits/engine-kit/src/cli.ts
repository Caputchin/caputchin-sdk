...
import { selfCheck, smokeCase, type SelfCheckCase, type SelfCheckReport } from '@caputchin/replay-selfcheck';
...
function buildCases(args: Args): SelfCheckCase[] {
  ...
  // Always probe the smoke case first (empty trace must return a Verdict, not
  // crash). Use the kit's own empty envelope so a reducer-lane run's decoder
  // accepts it.
  const cases: SelfCheckCase[] = [smokeCase(encodeTrace([]))];
