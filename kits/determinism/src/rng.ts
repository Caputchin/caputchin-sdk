// Deterministic PRNG. Algorithm: sfc32 (Small Fast Counter, 128-bit state) -
// fast, statistically strong, and built ONLY from int32 ops (`| 0`, `^`, `>>>`,
// `<<`, `+`), which are bit-identical across every JS engine and every V8 roll.
// The one float op, `(t >>> 0) / 4294967296`, divides a 32-bit unsigned by 2^32:
// an IEEE-754 correctly-rounded division, so also deterministic. No
// `Math.random`, no time, no platform dependence.
//
...
// `rngFromState`.
//
// Lives in the determinism kit alongside `capMath` as the other deterministic
// primitive a replay author/preset opts into. Takes a 128-bit seed as a plain
// four-word tuple (structurally the replay contract's `Seed`) so this kit stays
// a dependency-free leaf and never imports the contract.

/** Serializable PRNG state: four unsigned 32-bit words. Pass to {@link rngFromState} to resume an exact stream. */
export type RngState = readonly [number, number, number, number];

/**
 * Deterministic PRNG seeded from a 128-bit seed (four u32 words). Built on sfc32
 * (Small Fast Counter, 128-bit state) using only 32-bit integer operations, so
 * the stream is bit-identical across every JS engine and V8 version.
 *
 ...
 */
export function rng(seed: readonly [number, number, number, number]): Rng {
