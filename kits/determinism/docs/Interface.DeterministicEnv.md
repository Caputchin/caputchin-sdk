...
The seeded randomness + fixed clock a [withDeterministicEnv](Function.withDeterministicEnv.md) call
installs for the duration of one trapped function. The caller supplies the
seeded stream (e.g. this kit's own [rng](Function.rng.md)), so the trap itself stays
PRNG-agnostic.
