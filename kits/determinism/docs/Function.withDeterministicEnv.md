...
the transcendental `Math.*` are swapped to [capMath](Variable.capMath.md), `Math.random` is
the seeded `env.random`, and `Date.now` / `performance.now` return
`env.nowMs`.

This is the SCOPED, restorable counterpart to [makeDeterministic](Function.makeDeterministic.md): a
...
| Parameter | Type |
| ------ | ------ |
| `env` | [`DeterministicEnv`](Interface.DeterministicEnv.md) |
| `fn` | () => `T` |
