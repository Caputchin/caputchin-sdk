...
| <a id="trigger"></a> `trigger` | [`WidgetTrigger`](TypeAlias.WidgetTrigger.md) | When verification begins: `auto` on mount, `click` on the checkbox, `form-submit` when the enclosing form submits, or `manual` when you call `start()` yourself. |
