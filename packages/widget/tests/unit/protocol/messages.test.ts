...
describe('isIframeToWidget', () => {
  ...
  it('accepts manifest', () => expect(isIframeToWidget({ kind: 'manifest', seq: 0, gameId: 'g', preferredLayout: 'modal' })).toBe(true));
  it('accepts manifest with null preferredLayout', () => expect(isIframeToWidget({ kind: 'manifest', seq: 0, gameId: null, preferredLayout: null })).toBe(true));
