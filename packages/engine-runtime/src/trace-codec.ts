...
interface TraceEnvelope<A> {
  ...
  readonly v: number;
  /** engine-runtime version the recording ran under. */
...
export function decodeTrace<A>(trace: Uint8Array | string): readonly TickInput<A>[] {
  ...
  if (typeof parsed !== 'object' || parsed === null) {
    throw new Error('engine-runtime: malformed trace envelope');
  }
  const env = parsed as Record<string, unknown>;
  if (!Array.isArray(env.inputs)) {
    throw new Error('engine-runtime: trace envelope missing inputs[]');
  }
  const out: TickInput<A>[] = [];
  for (const raw of env.inputs) {
    if (typeof raw !== 'object' || raw === null) {
      throw new Error('engine-runtime: malformed trace input');
    }
    const e = raw as Record<string, unknown>;
    if (!Number.isInteger(e.tick) || (e.tick as number) < 0) {
      throw new Error('engine-runtime: malformed trace input tick');
    }
